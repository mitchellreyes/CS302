 ...
 *
 * @version 1.00 (29 September 2015)
 *
...
void QkSorter::quickSort(int first, int last)
{
  int breaker;
    if(first < last)
    {
        breaker = partition(first, last);
        quickSort(first, breaker);  
        quickSort(breaker+1, last);
    }
}

int QkSorter::partition(int first, int last)
{
    //SimpleVector<DateType> helper(*this);
    DateType firstObj, compObj, temp;
    getValueAt(first, firstObj);
    int firstInc = first; 
    int lastInc = last; 

    for(lastInc = first+1; lastInc < last+1; lastInc++)
    {   
        getValueAt(lastInc, compObj);
        if(compareTo(compObj, firstObj) <= 0) 
        {
            firstInc += 1;
            getValueAt(firstInc, temp);
            setValueAt(firstInc, compObj);
            setValueAt(lastInc, temp);
        }
    }
    getValueAt(firstInc, temp);
    setValueAt(firstInc, firstObj);
    setValueAt(first, temp);
    return firstInc;
}
