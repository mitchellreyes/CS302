 ...
 *
 * @brainstormed with: Philip Smith
 *
...
void HeapClass<KeyType, DataType>::addItemHelper(HeapNode<KeyType, DataType> *&node, const KeyType &newKey,const DataType &newData)
{
	...
	{
		bool lFull = isFull(node->left);
		bool rFull = isFull(node->right);
		if( !lFull || (rFull && (getHeightHelper(node->left, 'L') == getHeightHelper(node->right, 'L')) ) )
		{
			addItemHelper(node->left, newKey, newData);
		}
		else
		{
			addItemHelper(node->right, newKey, newData);
		}
	}
}
...
*/
template<typename KeyType, typename DataType>
bool HeapClass<KeyType, DataType>::isFull(HeapNode<KeyType, DataType> *node)
{
	// check null?
